...
public class SqlInjectionLesson8 implements AssignmentEndpoint {
  ...
  public static void log(Connection connection, String query, String name, String auth_tan) {
    String action = query.replaceFirst("\\?", name).replaceFirst("\\?", auth_tan);
    ...
    try (PreparedStatement statement = connection.prepareStatement(logQuery, TYPE_SCROLL_SENSITIVE, CONCUR_UPDATABLE)) {
